...
with tf.compat.v1.Session(config=NetConfig_MIR_1K.session_conf) as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        print('Load weights')
        tf.compat.v1.train.Saver().restore(sess, NetConfig_MIR_1K.CKPT_PATH + '/checkpoint-15000')

        testFileName = "test_output/dsd100_084_mixture_short.wav"
        ...
        printstr = testFileName + ' nsdr=' + str(nsdr) + ' sir=' + str(sir) + ' sar=' + str(sar)
        print(printstr)

        soundfile.write(testFileName.replace(".wav",'') + '_test_accompaniments.wav', y_wav1_orig, 44100)
        soundfile.write(testFileName.replace(".wav",'') + "_test_vocal.wav", y_wav2_orig, 44100)